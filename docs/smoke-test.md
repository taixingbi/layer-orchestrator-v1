...
SSE does **not** include `{"type":"state",...}` phase events (those are logs/metrics only). Expect `request_id`, `rewrite`, `route`, optional `answer_delta`, `answer`, then **`done`** with the **same full JSON shape** as non-stream (`route`, `rewrite`, `answer`, `citations`, `follow_up_questions`, `latency_ms`, `usage`, `status: "ok"`) plus `"type": "done"`.
