...
| `output/` | Generated JSONL (`train.jsonl`, `val.jsonl`, `build-stats.json`) — committed for `layer-router-train-v1` train |

**Training** lives in sibling app [`layer-router-train-v1`](../../../layer-router-train-v1/README.md) (QLoRA DPO on LAN GPU).
...
After running [`gold-test/run-router-eval.sh`](../gold-test/run-router-eval.sh), rebuild so **rejected** comes from real mismatches in `router-eval/gold-test/result/*.csv`:

```bash
ROUTER_PROMPT_VERSION=router-v2.00 bash router-eval/gold-test/run-router-eval.sh
bash router-eval/dpo-router/run-build-dpo.sh
```
...
```bash
ORCHESTRATOR_URL=http://192.168.86.179:30184 FETCH_LIVE=1 bash router-eval/dpo-router/run-build-dpo.sh
```
...
```bash
INCLUDE_SEED_FAQ=1 INCLUDE_HACK=1 bash router-eval/dpo-router/run-build-dpo.sh
```
...
| `OUTPUT_DIR` | `router-eval/dpo-router/output` | Output JSONL directory |
...
Dataset is built here; training runs in **[layer-router-train-v1](../../../layer-router-train-v1/README.md)**.

```bash
# 1) Build dataset (layer-orchestrator-v1)
bash router-eval/dpo-router/run-build-dpo.sh

# 2) Train (layer-router-train-v1 on GPU node 173/176)
cd ../layer-router-train-v1
...
- [layer-router-train-v1](../../../layer-router-train-v1/README.md) — QLoRA DPO training on LAN GPU
